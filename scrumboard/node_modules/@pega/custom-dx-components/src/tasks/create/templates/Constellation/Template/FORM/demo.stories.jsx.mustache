...
import {{COMPONENT_CLASS_NAME}} from './index.jsx';
...
{{! pick new delimiteres for mustache }}
{{=<% %>=}}
...
<%={{ }}=%>     {{! revert delimiteres for mustache }}
...
export const base{{COMPONENT_CLASS_NAME}} = () => {
  const props = {
    NumCols: 1,
  ...
  return (
    <>
      <{{COMPONENT_CLASS_NAME}} {...props}>{regionAChildren}</{{COMPONENT_CLASS_NAME}}>
    </>
  );
};
